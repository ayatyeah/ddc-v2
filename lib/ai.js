...
// ── Разделение труда между провайдерами ──────────────────────────────────────
// Осознанно делим задачи, чтобы оба ИИ были загружены по своим сильным сторонам,
// а не «Gemini один за всё»:
//   • OpenAI (ChatGPT) — «МОЗГ» интерактива: команды ассистента, голос, ответы RAG,
//     генерация документов, умные ответы в чатах. Отвечает быстро, понимает свободные
//     формулировки — поэтому у него ПРИОРИТЕТ во всём, что видит и с чем говорит человек.
//   • Gemini — «АНАЛИТИК» фоновых пачек: мультиязычный дайджест новостей и разбор заявок
//     (большой контекст, бесплатная квота, не жалко токенов). OpenAI здесь — страховка.
//
// aiBrain — интерактивный интеллект: OpenAI первым, Gemini как фолбэк (= прежний aiText,
// но имя явно говорит о роли; используем в ассистенте/чатах/документах).
async function aiBrain(prompt, opts = {}) { return aiText(prompt, opts); }

// aiBatch — тяжёлая фоновая аналитика: Gemini первым (большой контекст/квота),
// OpenAI — фолбэк, если Gemini упал/исчерпал лимиты. Так задачи разведены между провайдерами.
async function aiBatch(prompt, { maxTokens = 3000 } = {}) {
  if (GEMINI_KEYS.length) {
    try { return await callGemini(prompt, maxTokens); }
    catch (e) { console.error('aiBatch(gemini):', e.message, OPENAI_KEY ? '— пробую OpenAI' : ''); }
  }
  if (OPENAI_KEY) return await callOpenAIText(prompt, { json: true });
  throw new Error('Нет доступного ИИ-провайдера для аналитики');
}
...
module.exports = { GEMINI_KEYS, GEMINI_MODEL, callGemini, OPENAI_KEY, OPENAI_MODEL, callOpenAI, aiText, aiBrain, aiBatch, embed, transcribeAudio };
