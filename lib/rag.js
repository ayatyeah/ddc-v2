...
// ACL для результатов поиска. Индекс общий на всех, но заявки (отпуск/больничный с деталями),
// личные документы и задачи — приватные: без фильтра любой сотрудник через глобальный поиск и
// «Спроси у ДиДи» вытаскивал сниппеты чужих. admin/manager видят всё; остальные — только своё.
// Общие виды (wiki/news/person/service/event) доступны всем сотрудникам и не фильтруются.
const PRIVATE_KINDS = new Set(['document', 'request', 'task']);
async function filterHitsByAccess(hits, admin) {
  if (!hits || !hits.length) return hits || [];
  if (admin && ['admin', 'manager'].includes(admin.role)) return hits;
  const me = admin ? admin.id : null;
  const priv = hits.filter((h) => PRIVATE_KINDS.has(h.kind));
  if (!priv.length) return hits;
  const ids = { document: [], request: [], task: [] };
  for (const h of priv) ids[h.kind].push(Number(h.ref_id));
  const owners = { document: {}, request: {}, task: {} };
  const load = async (kind, sql) => {
    if (!ids[kind].length) return;
    try { (await db.query(sql, [ids[kind]])).rows.forEach((r) => { owners[kind][r.id] = r; }); }
    catch { /* таблицы может не быть — тогда скрываем всё этого вида */ }
  };
  await load('document', `SELECT id, author_id FROM documents WHERE id = ANY($1)`);
  await load('request', `SELECT id, author_id FROM requests WHERE id = ANY($1)`);
  await load('task', `SELECT id, assignee_id, created_by FROM tasks WHERE id = ANY($1)`);
  return hits.filter((h) => {
    if (!PRIVATE_KINDS.has(h.kind)) return true;
    const o = owners[h.kind][Number(h.ref_id)];
    if (!o || me == null) return false;
    if (h.kind === 'task') return o.assignee_id === me || o.created_by === me;
    return o.author_id === me;   // document, request
  });
}

module.exports = { cosine, reindexAll, semanticSearch, prefixSearch, removeFromIndex, filterHitsByAccess, KIND_LABEL, indexReady: () => aiIndexReady };
