...
async function seedDemo() {
  try {
    ...
    // Переписку общего канала сеет отдельный идемпотентный seedTeamChat() (ниже) —
    // он же чистит тестовый мусор в уже поднятой БД.
    console.log('✓ Демо-данные: 7 сотрудников (2 начальника), задачи, новости, заявки, 2 опроса, события, брони');
  } catch (e) { console.error('seedDemo:', e.message); }
}

// Живой «Общий канал»: реалистичная рабочая переписка команды (тон рабочий и строгий,
// без тестового «салам/как вы/test»). Времена разнесены на «вчера днём → сегодня утром».
// Идемпотентно: если маркерное (первое) сообщение уже есть — повторно не сеем.
const TEAM_CHAT = [
  ['n.sagatov',    'Коллеги, напоминаю: планёрка отдела разработки завтра в 11:00. Подготовьте краткие статусы по своим задачам.', 1555],
  ['d.akhmetov',   'По рефакторингу ядра расчётов — модуль покрыт тестами, готовлю PR. К обеду отправлю на ревью.', 1510],
  ['n.sagatov',    'Принято. Проверь обратимость миграций: в прошлый раз откат занял слишком много времени.', 1498],
  ['a.zhumabekova','Дашборд по транзакциям в регионах собран, данные за июнь подтянуты. Демо назначила на четверг в 15:00.', 1440],
  ['t.ospanov',    'Пайплайн для нового сервиса настроен, сборка проходит за 4 минуты. Деплой в прод — завтра в 14:00, в нерабочее окно.', 1422],
  ['a.kasymova',   'Тимур, перед деплоем пришлите чек-лист ИБ. С понедельника двухфакторная аутентификация обязательна для всех сервисов.', 1408],
  ['t.ospanov',    'Принято, чек-лист приложу к релизу.', 1401],
  ['e.tursynov',   'Отчёт по инцидентам ИБ за месяц готов: критичных — ноль, две попытки перебора паролей заблокированы автоматически. Сводка в разделе «Документы».', 322],
  ['a.kasymova',   'Хорошая работа. Добавьте в отчёт помесячную динамику — руководство просило тренд.', 306],
  ['m.serikkyzy',  'За неделю обработали 1 240 обращений на 1477, среднее время ответа — 3 минуты. Очередь разобрана.', 190],
  ['n.sagatov',    'Отличные цифры, спасибо команде поддержки 👍', 178],
  ['a.zhumabekova','Добавлю обращения 1477 в дашборд отдельным виджетом — будет видно нагрузку в реальном времени.', 168],
  ['d.akhmetov',   'PR по расчётам готов к ревью, нужно два аппрува перед мёржем. Ссылку приложил к задаче.', 62],
  ['n.sagatov',    'Беру на ревью. Смёржим после зелёного CI.', 44],
];
async function seedTeamChat() {
  try {
    const usernames = [...new Set(TEAM_CHAT.map(([u]) => u))];
    const rows = (await db.query(`SELECT id, username, full_name FROM users WHERE username = ANY($1)`, [usernames])).rows;
    if (!rows.length) return;                          // демо-сотрудников ещё нет — нечего сеять
    const ids = {}; const names = {};
    for (const r of rows) { ids[r.username] = r.id; names[r.username] = r.full_name; }
    const marker = TEAM_CHAT[0][1];                    // тело первого сообщения = маркер
    const seeded = (await db.query(
      `SELECT 1 FROM messages WHERE recipient_id IS NULL AND chat_id IS NULL AND body = $1 LIMIT 1`, [marker])).rows.length;
    if (seeded) return;                                // уже засеяно — идемпотентно, ничего не трогаем
    // Разовая чистка общего канала: убираем тестовый мусор и прежние демо-реплики,
    // чтобы канал выглядел живым. Строго ограничено общим каналом и точными строками.
    await db.query(
      `DELETE FROM messages WHERE recipient_id IS NULL AND chat_id IS NULL AND (
         lower(btrim(body)) = ANY($1) OR body = ANY($2))`,
      [['салам', 'салам!', 'как вы', 'как дела', 'привет', 'тест', 'test', 'realtime test', 'ping', 'проверка', 'qwe', 'asd', 'ok', 'ок'],
       ['Всем привет! Рад видеть команду в новом портале 👋', 'Привет! Дашборд по транзакциям почти готов, скоро покажу.', 'Коллеги, деплой нового сервиса завтра в 14:00.']]);
    for (const [u, body, minAgo] of TEAM_CHAT) {
      if (!ids[u]) continue;
      await db.query(
        `INSERT INTO messages (author_id, author_name, recipient_id, chat_id, body, created_at)
         VALUES ($1,$2,NULL,NULL,$3, now() - ($4 || ' minutes')::interval)`,
        [ids[u], names[u], body, minAgo]);
    }
    console.log('✓ Общий канал: засеяна реалистичная переписка команды, тестовый мусор очищен');
  } catch (e) { console.error('seedTeamChat:', e.message); }
}
