...
// Парсинг CV (PDF/DOCX) под таймаутом: .docx — это zip, и подложенная «архивная бомба» в
// пределах 5 МБ распаковывается в гигабайты, вешая парсер. Таймаут ограничивает время такой
// операции (полная защита памяти — изолированный воркер, это инфра-уровень; см. отчёт).
...
async function extractCvFileText(fileRow) {
  ...
  try {
    ...
    if (ext === 'pdf') {
      ...
      const data = await withTimeout(pdf(await fs.promises.readFile(p)), 15000, 'pdf-parse');
      return (data.text || '').replace(/\s+\n/g, '\n').trim().slice(0, 7000);
    }
    if (ext === 'docx') {
      const mammoth = require('mammoth');
      const r = await withTimeout(mammoth.extractRawText({ path: p }), 15000, 'mammoth');
