...
export default function Showcase() {
  ...
  useEffect(() => {
    ...
    // Инициализация модели (WebGL-контекст, компиляция тяжёлого шейдера стекла, сборка геометрии)
    // — синхронная работа на главном потоке ~0.5с. Попав в середину пролёта фона (navEase 1.1с),
    // она даёт видимый фриз на движущемся фоне. Поэтому если переход ИДЁТ (router выставил
    // __ddcFlyUntil), ждём его конца — тогда компиляция ляжет на уже статичный фон и незаметна.
    // При прямом заходе на /uslugi пролёта нет → стартуем по базовой задержке (после первой
    // отрисовки: плашки/текст сразу, модель — фейдом следом). Это замена прежнего «магического»
    // числа развязкой по факту перехода.
    const base = perf.mobile ? 900 : 420;
    const flyUntil = window.__ddcFlyUntil || 0;
    const delay = Math.max(base, flyUntil - performance.now() + 140);   // до конца пролёта + запас
    const tid = setTimeout(init, delay);
    return () => {
      cancelled = true;
      clearTimeout(tid);
