...
export default function Background3D({ onReady }) {
  ...
  useEffect(() => {
    ...
    // (pointer: coarse) = ОСНОВНОЙ ввод пальцем (телефон/планшет). Только там адресная строка
    // меняет высоту вьюпорта и палец мешает скроллу. Ноутбук с тач-экраном (pointer: fine)
    // сохраняет и обычный размер канвы, и перетаскивание мышью.
