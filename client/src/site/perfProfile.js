...
// Потолок разрешения рендера. Десктоп — без «мыла», вплоть до 2×. ТЕЛЕФОН — исключение:
// физический DPR 2.5–3× при слабом fill-rate GPU превращает полноэкранный 3D-фон в печку
// (нагрев + троттлинг + фризы даже на iPhone 13). Фон декоративен и частично размыт —
// 1.5× на глаз почти неотличим, а пикселей в ~1.8 раза меньше; на слабых — 1.25×.
const dprCap = mobile ? (lite ? 1.25 : 1.5) : engine === 'gecko' ? 1.75 : 2.0;
