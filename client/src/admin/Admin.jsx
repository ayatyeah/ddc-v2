...
const VacanciesAdmin = lazy(() => import('./VacanciesAdmin.jsx'));
...
const Broadcasts = lazy(() => import('./Broadcasts.jsx'));

const ROLE_LABEL = { admin: 'Администратор', manager: 'Начальник отдела', staff: 'Сотрудник', editor: 'Редактор', viewer: 'Просмотр' };
const TITLES = { dashboard: 'Дашборд', leads: 'Заявки', ai: 'ИИ-аналитика', analytics: 'Аналитика', webviews: 'Веб-аналитика', careers: 'Карьера', system: 'Мониторинг', security: 'Безопасность', wiki: 'База знаний', broadcast: 'Рассылки', services: 'Услуги', news: 'Новости', history: 'История', users: 'Пользователи' };
...
function Ico({ name, size = 20 }) {
  const p = {
    ...
    broadcast: <><path d="M3 11l18-7-7 18-2.5-7.5z" /><path d="M11.5 12.5L21 4" /></>,
...
export default function Admin() {
  ...
  const items = [
    ...
    { id: 'webviews', show: isAdmin || isManager },
    ...
    { id: 'broadcast', show: isAdmin || isManager },
      ...
      <div className="adm-body">
        ...
        <main className="adm-main" key={tab}>
          <Suspense fallback={<div className="adm-hint">Загрузка…</div>}>
            ...
            {tab === 'webviews' && (isAdmin || isManager) && <WebAnalytics onAuthLost={() => setState('login')} />}
            {tab === 'careers' && (isAdmin || isManager) && (<>
              <VacanciesAdmin onAuthLost={() => setState('login')} />
              <div style={{ height: 28 }} />
              <Careers onAuthLost={() => setState('login')} />
            </>)}
            ...
            {tab === 'broadcast' && (isAdmin || isManager) && <Broadcasts onAuthLost={() => setState('login')} isAdmin={isAdmin} />}
